 ...
 *
 */
public class NameFilterProcessor implements FileFilterProcessor {
	...
	public File[] getFileList(final AbstractFileFilter filter, final String path) {
		...
		if (filter.isNegate()) { 
			fileFilter = new NotFileFilter(new NameFileFilter(filter.getFilter()));
		}
		else {
			fileFilter = new NameFileFilter(filter.getFilter());
		}
